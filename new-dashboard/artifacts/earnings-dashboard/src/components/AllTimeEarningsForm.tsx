    ...
    <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4">
      <div className="bg-card rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="flex justify-between items-center p-5 border-b border-border">
          <h2 className="text-xl font-bold text-foreground">Ghostdash</h2>
          <button onClick={() => setAllTimeEarningsFormOpen(false)} className="text-muted-foreground hover:text-foreground">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-5 flex flex-col gap-5">
          ...
          <div className="bg-muted rounded-lg p-4">
            <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-3">Thème</h3>
            ...
            {fields.map(({ key, label, type }) => (
              <div key={key}>
                ...
                <input
                  ...
                  className="w-full border border-border rounded-md px-3 py-2 text-sm bg-background text-foreground outline-none focus:border-primary transition-colors"
                />
              </div>
            ))}
          </div>
        ...
        <div className="p-5 border-t border-border flex flex-wrap gap-3 justify-end items-center bg-muted/50 rounded-b-xl">
          ...
          <button onClick={() => setAllTimeEarningsFormOpen(false)} className="px-4 py-2 text-sm font-medium text-muted-foreground hover:text-foreground">
            Close
          </button>
          <button onClick={handleCalculate} className="px-6 py-2 bg-primary hover:bg-primary/90 text-white text-sm font-bold rounded-full transition-colors shadow-sm">
            CALCULATE
          </button>
        </div>
      </div>
