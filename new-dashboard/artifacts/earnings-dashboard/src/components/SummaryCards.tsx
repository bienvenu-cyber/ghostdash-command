...
export function SummaryCards() {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      {cards.map((card, i) => (
        <div key={i} className="bg-card border border-border rounded-xl p-6 shadow-[0_4px_12px_rgba(0,0,0,0.3)] hover:border-primary/30 transition-colors group relative">
          
          <div className="flex justify-between items-center mb-1">
            <h3 className="text-sm font-medium text-muted-foreground">{card.label}</h3>
            {editMode && <Edit2 className="w-3.5 h-3.5 text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity" />}
          </div>
          
          <div className="mb-2">
            {editMode ? (
              <div className="relative">
                <span className="absolute left-0 top-1/2 -translate-y-1/2 text-2xl font-bold text-card-foreground">$</span>
                <Input 
                  value={card.value}
                  onChange={(e) => handleStatChange(card.key as any, e.target.value)}
                  onBlur={(e) => handleBlur(card.key as any, e.target.value)}
                  className="pl-6 text-3xl font-bold h-12 bg-muted border-border text-card-foreground tabular-nums rounded-lg focus-visible:ring-primary focus-visible:border-primary"
                />
              </div>
            ) : (
